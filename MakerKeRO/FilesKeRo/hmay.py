...
async def lllocj7865j(client, message):
    ...
    if chek.status == ChatMemberStatus.OWNER or message.from_user.id == OWNER_ID or message.from_user.username in OWNER:
        ...
        return await message.reply_text(f"عذرا سيدي{message.from_user.mention} هذا الامر لا يخصك")
...
async def gigshkdnnj(client, message):
    ...
    if await set_join_must(client, message):
     return
    if message.chat.id in alkl:
      return await message.reply_text(f"عذرا سيدي [{message.from_user.mention}] الامر معطل من قبل مالك الجروب ✅")
...
async def lockKero(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == OWNER_ID or message.from_user.username in OWNER:
       ...
       return await message.reply_text(f"عذرا سيدي{message.from_user.mention}\n هذا الامر لا يخصك")



        
@Client.on_message(filters.command(["فتحي السب"], "") & filters.group, group=12345667)
...
async def deleeon(client, message):
   ...
   if "احا" in message.text:
       await message.delete()
       await message.reply_text(f"• عذراً سيدي ↤︎「 {message.from_user.mention}  」\n • تم قفل ارسال السب هنا .")
...
@Client.on_message(filters.command(["تليسجراف", "تيليغراف", "ميدييا"], ""), group=973) 
...
@Client.on_message(filters.reply & filters.regex("همسة") & filters.group, group=579)
...
async def send_hms(client, message): 
  ...
  if waiting_for_hms:    
    ...
    await client.send_message(chat_id=in_id, text=f"تم استلام همسة جديده \nلروئية الهمسة في الزر بالاسفل \nفقط هذا الشخص  [{user_mention}] الذي يستطيع رؤيتها 🔐", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("- اضغط لرؤية الهمسة ", callback_data = "hms_answer")],[InlineKeyboardButton("مستلم الهمسة", url=f"tg://openmessage?user_id={to_id}")],[InlineKeyboardButton("مرسل الهمسة", url=f"{from_url}")]]))     
    waiting_for_hms = False
...
def display_hms(client, callback):  
  ...
  if hmses.get(str(who_id)) is not None:
    ...
    callback.answer( "توقف عن اللعب لما لا يخصك", show_alert = True )
...
@Client.on_message(filters.command(["تقيبيسييد"], ""))
...
@Client.on_message(filters.command(["مبسح الميبقيدين"], ""))
...
@Client.on_message(filters.command(["المقيسيدين"], "")) 
...
banned_users = []
@Client.on_message(filters.command(["حؤيؤيؤظر", "طؤيؤرد"], ""), group=9764)
...
@Client.on_message(filters.command(["مسح المحظؤيؤؤيؤورين"], ""), group=9738)
...
@Client.on_message(filters.command(["اليؤغاء حظيؤر","/unbيؤيan"], ""), group=9765)
...
@Client.on_message(filters.command(["طيرد اليبوتات"], "") & filters.group, group=97365)
async def ban_bots(client, message):
    ...
    if count > 0:
        await message.reply_text(f"تم طيؤيرد {count} بوت بنجاح✅")
    else:
        await message.reply_text("لا توجد بوتات لطردها.")
  
@Client.on_message(filters.command(["تليؤيؤيجراف","/telegrapيؤيؤh", "/tmيؤؤي", "/tgؤيؤm"], ""), group=973) 
async def telegraph(client, message):
    ...
    try:
        ...
        button_s = InlineKeyboardMarkup([[InlineKeyboardButton("فيؤتح الرؤسيؤابط 🔗", url=f"https://telegra.ph{response[0]}")]])
        await message.reply(f"**اليؤؤرابط »**\n`https://telegra.ph{response[0]}`",disable_web_page_preview=True,reply_markup=button_s)
    finally:
        os.remove(download_location)

@Client.on_message(filters.command(["اليؤسغاء تثبيؤيت","غيسؤ ث"], ""), group=97365) 
async def unpin_message(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == OWNER_ID or message.from_user.username in OWNER:
        ...
        await message.reply_text(f"عذرا سيدي{message.from_user.mention} هذا الامر لا يخصك")

@Client.on_message(filters.command(["تثبييسيؤسيؤت", "يسيؤث"], ""), group=97354) 
...
async def mute_user_from_username(client, message):
    ...
    if user_id not in muted_users[group_id]:
        ...
        await message.reply_text(f"تم كتم العضو {user_mention} بنجاح.", reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("الغياء الكتم", callback_data=f"unmute{user_id}")]])
        )
    else:
        await message.reply_text("المستخدم مكتوم بالفعل.", reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("اليغاء الكتم", callback_data=f"unmute{user_id}")]])
        )
...
@Client.on_message(filters.command(["ايلغاء اليسكتم", "ايلغاء كتم"], ""), group=9735544576)
